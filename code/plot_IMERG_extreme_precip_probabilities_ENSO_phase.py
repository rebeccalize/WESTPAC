...
def map_composite_data(data, lats, lons, outfile, region, cmap, bounds, label,contour_or_grid,dtype=None):
	...
	plt.text(0.01, 1.01, label, transform=ax.transAxes, fontsize=9)
...
for ensolabel in ['elnino','lanina']: #   ,'lanina'
	

	infile = "gpm-imerg.precip_composite.percentage_of_days_exceeding_95th_percentile.ENSO."+ensolabel+"_years_1.0_ONI_threshold.*.txt"
	...
	no_days = infilename[108:112]

	print no_days
	
	outfile = "gpm-imerg.probability_of_extreme_rainfall.percentage_of_days_exceeding_95th_percentile_of_climatology.ENSO."+ensolabel+"_events.Aug-July.TRMM_grid.MONTHLY_percentiles.png"
	
	
	if ensolabel == 'elnino':
		label = "Extreme Precip Probability during El Nino Events (Aug - July) ("+str(no_days)+")"
		
	elif ensolabel == 'lanina':
		label = "Extreme Precip Probability during La Nina Events (Aug - July) ("+str(no_days)+")"
