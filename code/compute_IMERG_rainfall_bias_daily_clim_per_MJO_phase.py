...
IMERG_CLIM_FILE = IMERG_CLIM_DIR+"GPM_IMERG.daily_climatology.2001-2019.nc"
...
for MJO in [6]: #1,2,3,4,5,6,7,8
	...
	imerg_bias_comp = np.zeros((3600,1800)) #imerg grid - use this array to compute the average precip per day in this MJO phase
	imerg_bias_array = np.zeros((3600,1800,1)) #use vstack with this to hold the bias values, as don't want to sum; imerg grid - use this array to hold the sum of the daily precip on days in this MJO phase
	
	no_days = 0 #number of days included in thesum, in order to compute the mean 

	for year in years: #
	
		imerg_file = IMERG_DIR+"GPM_IMERG_3B-DAY.MS.MRG.3IMERG."+str(year)+".DAILY.V06.nc"
		print imerg_file
	...
	np.savetxt("gpm-imerg_precip_composite.daily_rainfall_bias_from_daily_climatology.MJO_phase_"+str(MJO)+"."+str(years[0])+"-"+str(years[-1])+"."+str(no_days).zfill(4)+"-days.using_precipitationCal.txt",  imerg_bias_comp[:,:], '%.4f')
