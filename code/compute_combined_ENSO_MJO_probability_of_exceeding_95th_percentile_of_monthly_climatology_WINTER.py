...
def compute_probabilities(year_array, MJO):
	...
	for m,i in zip([10,11,12,1,2,3],[0,1,2,3,4,5]):
		...
		if year_array == ENyears:
			...
			IMERG_CLIM_FILE = IMERG_CLIM_DIR+"GPM_IMERG_DAILY.TRMM_grid.ElNinoYears.*.m"+str(m).zfill(2)+".95th_percentile.nc"
			IMERG_CLIM_FILE = glob.glob(IMERG_CLIM_FILE)[0]
			print IMERG_CLIM_FILE
				
		elif year_array == LNyears:
			IMERG_CLIM_FILE = IMERG_CLIM_DIR+"GPM_IMERG_DAILY.TRMM_grid.LaNinaYears.*.m"+str(m).zfill(2)+".95th_percentile.nc"
			print IMERG_CLIM_FILE
			IMERG_CLIM_FILE = glob.glob(IMERG_CLIM_FILE)[0]
	...
	np.savetxt("gpm-imerg.precip_composite.percentage_of_days_exceeding_95th_percentile.COMBINED_ENSO_MJO.MJO_phase_"+str(MJO)+"."+enso_label+"."+str(total_no_days).zfill(4)+"-days.TRMM_grid.Oct-March.MONTHLY_ENSO_YEARS_ONLY_percentiles.txt",  perc_days_exc_pctl[:,:], '%.4f')
	
						
#compute_probabilities(ENyears,12)
#compute_probabilities(ENyears,34)
#compute_probabilities(ENyears,56)
#compute_probabilities(ENyears,78)

#compute_probabilities(LNyears,12)
#compute_probabilities(LNyears,34)
#compute_probabilities(LNyears,56)
compute_probabilities(LNyears,78)			
