...
for MJO in [7]: #1,2,3,4,5,6,7,8
	...
	np.savetxt("gpm-imerg.WINTER_Oct-March.precip_composite.daily_rainfall_bias_from_daily_climatology.MJO_phase_"+str(MJO)+"."+str(years[0])+"-"+str(years[-1])+"."+str(no_days).zfill(4)+"-days.lowerres_TRMM_grid.txt",  imerg_bias_comp[:,:], '%.4f')
	...
	np.savetxt("gpm-imerg.WINTER_Oct-March.CONSISTENCY.precip_composite.daily_rainfall_bias_from_daily_climatology.MJO_phase_"+str(MJO)+"."+str(years[0])+"-"+str(years[-1])+"."+str(no_days).zfill(4)+"-days.using_precipitationCal_lowerres_TRMM_grid_v2.txt",  consistency[:,:], '%.4f')
