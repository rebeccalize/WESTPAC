...
def map_composite_data(data, lats, lons, outfile, region, cmap, bounds, label1,contour_or_grid,dtype=None):
	...
	plt.text(0.01, 1.01, label, transform=ax.transAxes, fontsize=10)
	#plt.text(0.75, 0.8, label2, transform=ax.transAxes,fontsize=10)
...
ff_imerg = Dataset("/gws/nopw/j04/klingaman/emerton/GPM-IMERG/GPM_IMERG_3B-DAY.MS.MRG.3IMERG.2001.DAILY.V06_regridded_to_TRMM_grid.nc",'r')
...
for ENSO in ['elnino', 'lanina']:	

	for osc in ['MJO']: #'BSISO'
	
		for phase in [12,34,56,78]: #2,6
	
			infile = "gpm-imerg.precip_composite.percentage_of_days_exceeding_95th_percentile.COMBINED_ENSO_"+osc+"."+osc+"_phase_"+str(phase)+"."+ENSO+"_years*MONTHLY_ENSO_YEARS_ONLY_percentiles.txt"
			...
			if osc == 'MJO':
				no_days = infilename[134:138]
				bounds = np.linspace(0, 20, 11)
				season = 'WINTER'
			
				if ENSO == "elnino":
					label = "MJO Phase "+str(phase)+" during El Nino winter  ("+str(no_days)+")"
				elif ENSO == "lanina":
					label = "MJO Phase "+str(phase)+" during La Nina winter  ("+str(no_days)+")"
				
			elif osc == 'BSISO':
				no_days = infilename[138:142]
				bounds = np.linspace(0, 20, 11)
				season = 'SUMMER'
			
				if ENSO == "elnino":
					label = "BSISO Phase "+str(phase)+" during summer after El Nino  ("+str(no_days)+")"
				elif ENSO == "lanina":
					label = "BSISO Phase "+str(phase)+" during summer after La Nina  ("+str(no_days)+")"
			...
			outfile = "gpm-imerg.probability_of_extreme_rainfall.percentage_of_days_exceeding_95th_percentile_of_climatology.COMBINED_"+osc+"_phase_"+str(phase)+"."+ENSO+"_years."+season+".TRMM_grid.MONTHLY_ENSO_YEARS_ONLY_percentiles.png"
