...
def map_composite_data(data, lats, lons, outfile, region, cmap, bounds, label,contour_or_grid,dtype=None):
	...
	if dtype == 'bias':
		...
		im.cmap.set_bad('white', alpha=0)  # set missing values to white/transparent
		cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04, ticks=bounds[0::2]) #, extend="max"
	...
	plt.text(0.78, 0.8, label, transform=ax.transAxes, fontsize=10)
...
for MJO in [12,34,56,78]: #2,6
	for ENSO in ['elnino', 'lanina']:
		
		infile = "gpm-imerg.precip_composite.daily_rainfall_bias_from_daily_climatology.MJO_phase_"+str(MJO)+"."+ENSO+"_years_1.0_ONI_threshold.*-days.txt"
		...
		no_days = infilename[114:118] #get the number of days out of the filename string
		print no_days
	
		outfile = "gpm-imerg.mean_daily_precip_bias_from_daily_climatology_composite.MJO_phase_"+str(MJO)+"."+ENSO+"_years_1.0_ONI_threshold.mmperday.png"
		...
		infile_consistency = "gpm-imerg.CONSISTENCY.precip_composite.daily_rainfall_bias_from_daily_climatology.MJO_phase_"+str(MJO)+"."+ENSO+"_years_1.0_ONI_threshold.*-days.txt"
		...
		outfile_consistency = "gpm-imerg.mean_daily_precip_bias_from_daily_climatology_composite.CONSISTENCY.MJO_phase_"+str(MJO)+"."+ENSO+"_years_1.0_ONI_threshold.mmperday.png"
